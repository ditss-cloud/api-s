...
const foldersToServe = fs.readdirSync(__dirname).filter(f => {
  ...
  return fs.statSync(full).isDirectory()
    && !['node_modules', 'page', 'src'].includes(f)
...
// === Clean URL handler: /folder/page → page/folder/page.html ===
app.get('/:folder/:page', (req, res, next) => {
  const filePath = path.join(__dirname, 'page', req.params.folder, `${req.params.page}.html`);
  if (fs.existsSync(filePath)) return res.sendFile(filePath);
  next();
});

// === Clean URL handler: /page → page/page.html ===
app.get('/:page', (req, res, next) => {
  const filePath = path.join(__dirname, 'page', `${req.params.page}.html`);
  if (fs.existsSync(filePath)) return res.sendFile(filePath);
  next();
});

// === Catch-all: /a/b/c → page/a/b/c.html ===
app.get('*', (req, res, next) => {
  const tryPath = path.join(__dirname, 'page', req.path + '.html');
